...
# we actually need to test without the config_plugin option sa well, so we are
# sure we explode in each case!!

my @tests = (
    {
        name => 'weaver.ini is broken',
        files => [
            path(qw(source weaver.ini)) => "[\@Def ault] ; this is clearly broken\n",
        ],
        has_config_plugin => 0,
